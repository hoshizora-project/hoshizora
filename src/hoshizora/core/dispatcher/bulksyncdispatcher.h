    ...
    struct BulkSyncDispatcher : Dispatcher<Kernel> {
        ...
        inline void push_tasks(Func f, ID *boundaries) {
            ...
            loop::each_thread(boundaries,
                              [&](u32 block_id, u32 numa_id, u32 thread_id, u32 lower, u32 upper) {
                                  tasks->emplace_back([=, &f]() {
                                      for (ID dst = lower; dst < upper; ++dst) {
                                          f(dst, block_id, numa_id);
                                      }
                                  });
                              });
        ...
        std::string run() {
            for (auto iter = 0u; iter < num_iters; ++iter) {
                ...
                if (iter == 0) {
                    push_tasks([kernel, prev_graph](ID src, u32 block_id, u32 numa_id) {
                ...
                // scatter and gather
                push_tasks([kernel, prev_graph, curr_graph](ID src, u32 block_id, u32 numa_id) {
                    for (ID i = 0, end = prev_graph->out_degrees(src, block_id); i < end; ++i) {
                        const auto dst = prev_graph->out_neighbors(src, block_id)[i];
                        const auto index = prev_graph->out_offsets(src, block_id) + i;
                        const auto forwarded_index = prev_graph->forward_indices[index];
                        curr_graph->e_data(forwarded_index/*, block_id*/)
                                = kernel.gather(src, dst,
                                                prev_graph->e_data(forwarded_index/*, block_id*/),
                                                kernel.scatter(src, dst, prev_graph->v_data(src, block_id),
                                                               *prev_graph),
                                                *prev_graph);
                    }
                }, prev_graph->out_boundaries);
                ...
                // sum and apply
                push_tasks([kernel, curr_graph, prev_graph](ID dst, u32 block_id, u32 numa_id) {
                    curr_graph->v_data(dst/*, block_id*/) = kernel.zero(dst, *prev_graph); // TODO
                    for (ID i = 0, end = prev_graph->in_degrees(dst, block_id); i < end; ++i) {
                        const auto src = prev_graph->in_neighbors(dst, block_id)[i];
                        const auto index = prev_graph->in_offsets(dst, block_id) + i;

                        curr_graph->v_data(dst/*, block_id*/) = kernel.sum(dst, src,
                                                                          curr_graph->v_data(dst/*, block_id*/),
                                                                          curr_graph->e_data(index/*, block_id*/),
                                                                          *prev_graph);
                    }

                    curr_graph->v_data(dst/*, block_id*/) = kernel.apply(dst,
                                                                        prev_graph->v_data(dst/*, block_id*/),
                                                                        curr_graph->v_data(dst/*, block_id*/),
                                                                        *prev_graph);
