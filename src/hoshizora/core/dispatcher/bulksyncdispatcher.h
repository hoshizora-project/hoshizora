...
namespace hoshizora {
    ...
    struct BulkSyncDispatcher : Dispatcher<Kernel> {
        ...
        template<class Func>
        inline void push_outbound(Func f) {
            auto bulk = new std::vector<std::function<void()>>();
            parallel::each_thread(prev_graph->out_boundaries,
        ...
        template<class Func>
        inline void push_inbound(Func f) {
            auto bulk = new std::vector<std::function<void()>>();
            parallel::each_thread(prev_graph->in_boundaries,
                                  [&](u32 numa_id, u32 thread_id, u32 lower, u32 upper) {
                                      bulk->emplace_back([=, &f]() {
                                          for (ID dst = lower; dst < upper; ++dst) {
                                              f(dst, numa_id);
        ...
        std::string run() {
            ...
            for (auto iter = 0u; iter < num_iters; ++iter) {
                ...
                if (iter == 0) {
                    push_outbound([kernel, prev_graph](ID src, u32 numa_id) {
                ...
                // scatter and gather
                push_outbound([kernel, prev_graph, curr_graph](ID src, u32 numa_id) {
                ...
                // sum and apply
                push_inbound([kernel, curr_graph, prev_graph](ID dst, u32 numa_id) {
