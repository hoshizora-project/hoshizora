...
namespace hoshizora::compress {
    ...
    static inline u8 ungap(const u8 *__restrict const in, const u32 prev, u32 *__restrict const out) {
        constexpr u32 o0 = 0;
    ...
    const static function<u8(const u8 *__restrict const, const u32,
                              u32 *__restrict const)> ungaps[256] = {
        ungap<0, 0, 0, 0, 0, 0, 0>,
        ungap<1, 0, 0, 0, 0, 0, 0>,
        ungap<0, 1, 0, 0, 0, 0, 0>,
        ungap<1, 1, 0, 0, 0, 0, 0>,
    ...
    static u32 single_decode(const u8 *__restrict const in, const u32 size, u32 *__restrict const out) {
        if (size > 0u) {
            u32 out_offset = 0;
            u32 in_offset = 0;
                ...
                for (u32 i = 0; i < N; i++) {
                    ...
                    _mm256_store_si256(reinterpret_cast<__m256ipc>(flags.data() + YMM_BYTE * (i * 2u)),
                                       _mm256_and_si256(reg,
                                                        _mm256_load_si256(
                                                                reinterpret_cast<__m256icpc>(mask8r[4]))));
            ...
            if (remain > 0u) {
                ...
                const u32 consumed = ungaps[in[in_offset]](
    ...
    static u32 single_traverse(const u8 *__restrict const in, const u32 size, Func f) {
        if (size > 0u) {
            u32 out_offset = 0;
            u32 in_offset = 0;
            const u32 n_blocks = size / LENGTH;
