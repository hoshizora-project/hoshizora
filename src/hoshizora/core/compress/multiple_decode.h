...
namespace hoshizora::compress {
        ...
        while (true) {
            ...
            if (len > THRESHOLD) {
                ...
                }
                {
                    ...
                    const auto proceeded = reinterpret_cast<const u8 *__restrict>(pfor->decodeArray(
    ...
    static void multiple_traverse(const u8 *__restrict const in, const u32 n_lists,
        ...
        while (true) {
            ...
            u32 len = offsets[i + 1] - offsets[i];
            if (len > THRESHOLD) {
                in_consumed += single_traverse(head, len, bind(f, placeholders::_1, i, placeholders::_2));
                i++;
            } else {
                u32 len_acc = 0;
                while (true) {
                ...
                {
                    size_t consumed = len_acc;
                    const auto proceeded = reinterpret_cast<const u8 *__restrict>(pfor->mapArray(
                            reinterpret_cast<const u32 *__restrict const>(head),
                            consumed/*dummy*/, buffer, consumed/*as len_acc*/,
